...
const AdminDashboard: React.FC = () => {
  ...
  const getRoleBadge = (role: UserRole) => {
    if (role === 'admin') return 'bg-purple-500/20 text-purple-600 dark:text-purple-300 border-purple-500/30';
    if (role === 'manager') return 'bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 border-indigo-500/30';
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0b0f19] dark:text-white transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-slate-500 text-sm ml-2 font-medium">Admin Portal</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/20 text-red-600 dark:text-red-300 font-semibold border border-red-500/30">
                System Admin
              </span>
            </button>
            <NotificationBell />
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Manage organization user accounts, roles, and department structures.</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            <div key={stat.label} className="bg-white border-slate-200 shadow-sm dark:bg-white/[0.03] dark:border-white/[0.06] border rounded-xl p-4 text-center animate-fade-in">
              <p className="text-2xl">{stat.icon}</p>
              <p className="text-2xl font-bold text-slate-900 dark:text-white tabular-nums mt-1">{stat.value}</p>
        ...
        {/* Tab Controls */}
        <div className="flex gap-1 p-1 bg-slate-100 border-slate-200 dark:bg-white/[0.03] dark:border-white/[0.06] border rounded-xl w-full sm:w-fit overflow-x-auto mb-6">
          <button
            onClick={() => setActiveTab('users')}
            className={`flex-shrink-0 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
              activeTab === 'users' ? 'bg-white text-slate-900 shadow-sm dark:bg-white/[0.08] dark:text-white' : 'text-slate-600 hover:text-slate-900 dark:text-slate-500 dark:hover:text-slate-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex-shrink-0 px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
              activeTab === 'departments' ? 'bg-white text-slate-900 shadow-sm dark:bg-white/[0.08] dark:text-white' : 'text-slate-600 hover:text-slate-900 dark:text-slate-500 dark:hover:text-slate-300'
            }`}
          >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                ...
                <form onSubmit={handleCreateUser} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border-slate-200 text-slate-900 dark:bg-white/[0.05] dark:border-white/10 dark:text-white border text-sm focus:outline-none focus:ring-2 focus:ring-red-500/50"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Assigned Role</label>
                    <select
                      value={userForm.role}
                      onChange={e => setUserForm(f => ({ ...f, role: e.target.value as UserRole }))}
                      className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border-slate-200 text-slate-900 dark:bg-white/[0.05] dark:border-white/10 dark:text-white border text-sm focus:outline-none focus:ring-2 focus:ring-red-500/50"
                    >
                      <option value="staff" className="bg-white dark:bg-[#0f1729]">💼 Staff (Task Worker)</option>
                      <option value="manager" className="bg-white dark:bg-[#0f1729]">👔 Manager (Task Assignee)</option>
                      <option value="admin" className="bg-white dark:bg-[#0f1729]">👑 System Admin</option>
                    </select>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      {departments.map(d => (
                        <option key={d.id} value={d.id} className="bg-white dark:bg-[#0f1729]">{d.name}</option>
                      ))}
                    </select>
                  </div>
            ...
            <div className="lg:col-span-3">
              <div className="bg-white border-slate-200 shadow-sm dark:bg-white/[0.03] dark:border-white/[0.08] border rounded-2xl p-6">
                <h2 className="text-base font-semibold text-slate-900 dark:text-white mb-4">All User Accounts</h2>
                <div className="space-y-2">
                  {users.map(u => (
                    <div
                      key={u.id}
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3.5 rounded-xl border bg-slate-50/80 border-slate-200 hover:bg-slate-100 dark:border-white/[0.06] dark:bg-white/[0.02] dark:hover:bg-white/[0.04] transition-all"
                    >
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                        <span className="text-xs text-slate-600 bg-slate-200 dark:text-slate-400 dark:bg-white/[0.04] px-2.5 py-1 rounded-md font-medium">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            <div className="lg:col-span-2">
              <div className="bg-white border-slate-200 shadow-sm dark:bg-white/[0.03] dark:border-white/[0.08] border rounded-2xl p-6 animate-fade-in">
                ...
                {deptFormSuccess && (
                  <div className="mb-4 px-4 py-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 text-sm animate-slide-up">
                    {deptFormSuccess}
                  </div>
                )}
                ...
                <form onSubmit={handleCreateDept} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border-slate-200 text-slate-900 dark:bg-white/[0.05] dark:border-white/10 dark:text-white border text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/50"
                    />
                  </div>
            ...
            <div className="lg:col-span-3">
              <div className="bg-white border-slate-200 shadow-sm dark:bg-white/[0.03] dark:border-white/[0.08] border rounded-2xl p-6">
                <h2 className="text-base font-semibold text-slate-900 dark:text-white mb-4">Organization Departments</h2>
                <div className="space-y-2">
                  {departments.map(d => (
                    <div
                      key={d.id}
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-4 rounded-xl border bg-slate-50/80 border-slate-200 hover:bg-slate-100 dark:border-white/[0.06] dark:bg-white/[0.02] dark:hover:bg-white/[0.04] transition-all"
                    >
                      <div>
                      ...
                      <span className="text-xs text-slate-600 bg-slate-200 dark:text-slate-400 dark:bg-white/[0.04] px-3 py-1 rounded-full border border-slate-200 dark:border-white/[0.06] font-medium">
