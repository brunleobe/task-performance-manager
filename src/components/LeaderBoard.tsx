    ...
    <div className="space-y-3">
      {sorted.map((entry, index) => {
        ...
        return (
          <div
            key={entry.user_id}
            className="group p-3 rounded-xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/10 transition-all duration-200 animate-slide-up"
            style={{ animationDelay: `${index * 80}ms` }}
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className={`text-lg font-bold ${rank.color}`}>{rank.emoji}</span>
                <span className="text-sm font-medium text-white">{entry.user_name ?? 'Staff Member'}</span>
              </div>
